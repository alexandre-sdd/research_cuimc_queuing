...
The choice $D=20$ below is purely illustrative.
Any calendar-day label would work; the point is only to show explicitly where the process stays
within the same day and where it rolls over to the next one.

The following table shows both coordinates on the same timeline:
\[
\begin{array}{c|cccccccc}
\text{epoch } n
& 0 & 1 & 2 & \cdots & S-1 & S & S+1 & \cdots \\
\hline
D
& 20 & 20 & 20 & \cdots & 20 & 21 & 21 & \cdots \\
s
& 0 & 1 & 2 & \cdots & S-1 & 0 & 1 & \cdots \\
t_n=(D,s)
& (20,0) & (20,1) & (20,2) & \cdots & (20,S-1) & (21,0) & (21,1) & \cdots
\end{array}
\]
...
\node[epoch] (t0) at (0,0) {$(20,0)$};
\node[epoch] (t1) at (3.0,0) {$(20,1)$};
\node[epoch] (t2) at (6.0,0) {$(20,2)$};
\node[epoch] (tdots) at (8.8,0) {$\cdots$};
\node[epoch] (tlast) at (11.8,0) {$(20,S\!-\!1)$};
\node[epoch] (u0) at (15.6,0) {$(21,0)$};
\node[epoch] (u1) at (18.6,0) {$(21,1)$};

\draw[->, thick] (t0) -- (t1);
\draw[->, thick] (t1) -- (t2);
\draw[->, thick] (t2) -- (tdots);
\draw[->, thick] (tdots) -- (tlast);
\draw[->, thick, red!70!black] (tlast) -- node[above, yshift=1pt, align=center] {$s=S-1$\\$\Rightarrow D \leftarrow D+1,\ s \leftarrow 0$} (u0);
\draw[->, thick] (u0) -- (u1);

\node[same_day, fit=(t0)(t1)(t2)(tdots)(tlast), label=above:{same day: $D=20$ fixed, $s$ advances}] {};
\node[next_day, fit=(u0)(u1), label=above:{next day: $D=21$}] {};
\end{tikzpicture}
}
\end{center}
