...
It is helpful to think of this calendar as an $H \times S$ table. The row index $r$ tells us
which day ahead we are looking at relative to the current day $D$: $r=0$ means ``today'',
$r=1$ means ``tomorrow'', and so on. The column index $m$ tells us which slot within that day we
are looking at: $m=0$ is the first slot of the day, $m=1$ the second, and so on until $m=S-1$.

Schematically, the rolling calendar has the following structure:
\begin{center}
\renewcommand{\arraystretch}{1.35}
\setlength{\tabcolsep}{7pt}
\begin{tabular}{|>{\raggedright\arraybackslash}m{2.9cm}|>{\centering\arraybackslash}m{2.2cm}|>{\centering\arraybackslash}m{2.2cm}|>{\centering\arraybackslash}m{1.6cm}|>{\centering\arraybackslash}m{2.4cm}|}
\hline
\rowcolor{gray!15}
\textbf{Day in horizon} & \shortstack{\textbf{slot 0}\\($m=0$)} & \shortstack{\textbf{slot 1}\\($m=1$)} & \textbf{$\cdots$} & \shortstack{\textbf{slot $S-1$}\\($m=S-1$)} \\
\hline
\cellcolor{blue!8}\shortstack[l]{\textbf{day $D$}\\($r=0$)} & $Y_t(0,0)$ & $Y_t(0,1)$ & $\cdots$ & $Y_t(0,S-1)$ \\
\hline
\cellcolor{blue!8}\shortstack[l]{\textbf{day $D+1$}\\($r=1$)} & $Y_t(1,0)$ & $Y_t(1,1)$ & $\cdots$ & $Y_t(1,S-1)$ \\
\hline
\cellcolor{gray!8}\textbf{later days} & $\vdots$ & $\vdots$ & $\ddots$ & $\vdots$ \\
\hline
\cellcolor{blue!8}\shortstack[l]{\textbf{day $D+H-1$}\\($r=H-1$)} & $Y_t(H-1,0)$ & $Y_t(H-1,1)$ & $\cdots$ & $Y_t(H-1,S-1)$ \\
\hline
\end{tabular}
\end{center}

So each row is a day in the rolling horizon, and each column is a slot inside that day.
...
\begin{tabular}{|>{\raggedright\arraybackslash}m{2.9cm}|>{\centering\arraybackslash}m{2.2cm}|>{\centering\arraybackslash}m{2.2cm}|>{\centering\arraybackslash}m{2.2cm}|>{\centering\arraybackslash}m{2.2cm}|}
\hline
\rowcolor{gray!15}
\textbf{Day in horizon} & \textbf{slot 0} & \textbf{slot 1} & \textbf{slot 2} & \textbf{slot 3} \\
\hline
\cellcolor{blue!8}\shortstack[l]{\textbf{day $D$}\\($r=0$)} &
\cellcolor{green!12}\textsf{open} &
\cellcolor{blue!12}$(1,0,\varnothing)$ &
\cellcolor{green!12}\textsf{open} &
\cellcolor{orange!18}$(2,0,\varnothing)$ \\
\hline
\cellcolor{blue!8}\shortstack[l]{\textbf{day $D+1$}\\($r=1$)} &
\cellcolor{blue!12}$(1,1,\varnothing)$ &
\cellcolor{green!12}\textsf{open} &
\cellcolor{orange!18}$(2,1,\varnothing)$ &
\cellcolor{green!12}\textsf{open} \\
\hline
\cellcolor{blue!8}\shortstack[l]{\textbf{day $D+2$}\\($r=2$)} &
\cellcolor{green!12}\textsf{open} &
...
This example is read row by row.
The first row is today's remaining schedule, the second row is tomorrow's schedule, and the third
row is the schedule for the day after tomorrow.
For instance, the entry in row $r=1$ and column $m=2$ is $(2,1,\varnothing)$, meaning that the
third slot of day $D+1$ is occupied by a class-2 patient who accepted a one-day delay and is not
scheduled to cancel before service.
...
So $Y_t(r,m)$ is simply the content of row $r$, column $m$ of that rolling calendar.
If
\[
Y_t(r,m)=0,
\]
