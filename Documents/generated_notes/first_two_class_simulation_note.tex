...
\begin{itemize}[nosep]
    ...
    \item $\phi_i$ governs eventual pre-appointment cancellation for class-$i$ patients,
...
\begin{itemize}[nosep]
    ...
    \item the patient's eventual pre-appointment cancellation behavior is governed by $\phi_2$,
...
C_2=\{\lambda_2,b_2(\tau),\phi_2,\xi_2(\tau)\}.
\]
So while the patient waits, the simulator uses the class-2 cancellation parameter $\phi_2$ to
apply a daily cancellation risk, and if the patient reaches service it uses $\xi_2(2)$ for
attendance.
...
\begin{itemize}[nosep]
    ...
    \item $\phi_i$: eventual probability that a class-$i$ patient booked with $\tau \ge 1$ cancels before the appointment day,
...
The simulator converts this class-level quantity into a daily cancellation probability
\[
q_i(\tau)=
\begin{cases}
0, & \tau=0,\\[0.4em]
1-(1-\phi_i)^{1/\tau}, & \tau\ge 1,
\end{cases}
\]
so that a patient who booked with delay $\tau$ and faces one cancellation trial at the end of each
pre-appointment day still has total cancellation probability $\phi_i$ before the appointment.
...
stored as $(i,\tau)$ cancels independently with probability $q_i(\tau)$.
...
\begin{itemize}[nosep]
    ...
    \item the total cancellation probability across those daily trials remains $\phi_i$.
\end{itemize}
