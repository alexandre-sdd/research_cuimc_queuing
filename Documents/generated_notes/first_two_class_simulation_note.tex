...
For instance, the entry in day-column $r=1$ and slot-row $m=2$ is $(2,1)$, meaning that slot $2$
of day $D+1$ is occupied by a class-2 patient who accepted a one-day delay.
Because the patient belongs to class $2$, the simulator knows to use the class-2 behavior object
$C_2=\{\lambda_2,b_2(\tau),\phi_2,\xi_2(\tau)\}$.
That means:
\begin{itemize}[nosep]
...
For example:
\begin{itemize}[nosep]
    \item $Y_{(20,0)}(1,0)=(1,1)$ means slot $m=0$ on day $21$ is booked by a class-1 patient who, when booking, accepted a one-day delay,
    \item $Y_{(20,0)}(2,1)=(2,2)$ means slot $m=1$ on day $22$ is booked by a class-2 patient who accepted a two-day delay.
\end{itemize}
