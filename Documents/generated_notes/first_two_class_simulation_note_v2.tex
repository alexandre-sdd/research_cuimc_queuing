...
\section{Purpose and Reader Map}
...
\subsection{What Changed from the Previous Note}
...
\addlinespace
Cancellation plot \(\phi_i\) and \(\tilde{\phi}_i(\tau,r)\) & how the baseline daily cancellation probability differs from the older delay/residual-delay function & scalar daily cancellation \(\phi_i\) as the baseline, with advanced \(\tilde{\phi}_i(\tau,r)\) mentioned but not used yet \\
\addlinespace
...
\section{Day-Level State}

At the start of day \(D\), after the daily cancellation step and before processing new arrivals,
define
\[
X_{i,r}^{D}
=
...
\subsection{Patient Audit Record}
...
\section{Patient Classes and Behavior}
...
\addlinespace
\(\phi_i\) & scalar daily cancellation probability for class-\(i\) scheduled patients \\
\addlinespace
...
\addlinespace
\(\xi_i(\tau)\) & no-show probability at appointment time if the patient has not canceled \\
\bottomrule
\end{tabular}
\normalsize
\end{center}

The baseline cancellation input is \(\phi_i\). It is applied directly once per day to every scheduled
class-\(i\) patient considered in the daily cancellation step. It is not converted from an eventual
...
\subsection{Working Behavior Profile}
...
\midrule
Daily demand & \(\lambda_1=3.5\) when \(\lambda=6.0\) and \(p=7/12\) & \(\lambda_2=2.5\) when \(\lambda=6.0\) and \(p=7/12\) \\
\addlinespace
Balking & low until a 21-day access threshold, then high & low until a 14-day access threshold, then high \\
\addlinespace
No-show & Green-Savin style exponential curve with low baseline and slow delay sensitivity & higher baseline and stronger delay sensitivity \\
\addlinespace
...
\subsection{No-Show Function}

No-show behavior uses the original accepted delay \(\tau\), not the residual delay \(r\) on a later
day. The current source-aligned profile uses the Green-Savin exponential-saturation form:
\[
\xi_i(\tau)
=
\gamma_{i,\max}
...
\begin{examplebox}[title={Delay-Sensitive No-Show Curves}, breakable=false]
\begin{center}
\begin{tikzpicture}
\begin{axis}[
    width=0.84\linewidth,
    ...
    ylabel={no-show probability \(\xi_i(\tau)\)},
    xmin=0,
    xmax=30,
    ymin=0,
    ymax=0.55,
...
\addplot[smooth, thick, color=FrameBlue, domain=0:30, samples=120] {0.31 - (0.31 - 0.01)*exp(-x/50)};
\addlegendentry{class 1}
\addplot[smooth, thick, color=FrameGold, domain=0:30, samples=120] {0.51 - (0.51 - 0.15)*exp(-x/9)};
\addlegendentry{class 2}
\end{axis}
\end{tikzpicture}
\end{center}

Class 2 starts at a higher no-show probability and reacts more strongly to delay. This gives the
two-class model a reason to track class counts separately even when FCFS is the only allocation
rule currently being analyzed.
\end{examplebox}
...
\begin{enumerate}[itemsep=0.35em]
    \item \textbf{Compute cancellations for the day.} Each already scheduled class-\(i\) patient independently cancels with daily probability \(\phi_i\).
    ...
    \item \textbf{Simulate no-shows and service for day \(D\).} Each remaining appointment scheduled for \(D\) no-shows with probability \(\xi_i(\tau)\), using its stored original delay.
...
\begin{keybox}[title={Future-Only Offers}]
New arrivals on day \(D\) cannot book day \(D\). Even if a morning cancellation opens capacity
today, baseline FCFS searches only \(r=1,\dots,H-1\). Same-day holes therefore affect today's
empty-slot metrics, not today's new offers.
\end{keybox}

\section{Day-Level Calendar Picture}
...
Suppose \(S=3\), \(H=4\), and after the cancellation step on day \(D\) the class-count state is:
...
\hline
\cellcolor{gray!10}\(X_{1,r}^{D}\) & \cellcolor{blue!12}\calendarentry{1} & \cellcolor{blue!12}\calendarentry{2} & \cellcolor{blue!12}\calendarentry{0} & \cellcolor{blue!12}\calendarentry{0} \\
\hline
\cellcolor{gray!10}\(X_{2,r}^{D}\) & \cellcolor{orange!18}\calendarentry{1} & \cellcolor{orange!18}\calendarentry{1} & \cellcolor{orange!18}\calendarentry{1} & \cellcolor{orange!18}\calendarentry{0} \\
\hline
\cellcolor{gray!10}\(N_r^D\) & \calendarentry{2} & \cellcolor{red!12}\calendarentry{3} & \calendarentry{1} & \calendarentry{0} \\
\hline
\cellcolor{gray!10}\(S-N_r^D\) & \cellcolor{green!12}\calendarentry{1} & \cellcolor{red!12}\calendarentry{0} & \cellcolor{green!12}\calendarentry{2} & \cellcolor{green!12}\calendarentry{3} \\
\hline
\end{tabular}
}
\end{center}

The current day \(r=0\) has one open slot, but it is not bookable by new arrivals. Tomorrow
...
\section{Worked Day-Level Example}
...
Take \(S=3\), \(H=4\), and the post-cancellation state from the previous table:
\[
N_0^D=2,\qquad N_1^D=3,\qquad N_2^D=1,\qquad N_3^D=0.
\]
Assume that the cancellation step has already removed one appointment, so the daily cancellation
counter is \(1\).

Daily arrivals are drawn once per class:
\[
M_1^D=2,\qquad M_2^D=1.
\]
The simulator randomizes the within-day order. Suppose the realized order is:
\[
\text{class 1},\quad \text{class 2},\quad \text{class 1}.
\]

The three offer decisions are:
\begin{center}
\small
\begin{tabular}{L{0.12\textwidth} L{0.18\textwidth} L{0.22\textwidth} L{0.32\textwidth}}
\toprule
\textbf{Arrival} & \textbf{Class} & \textbf{FCFS offer} & \textbf{Result} \\
\midrule
1 & 1 & \(r=2,\ \tau=2\) & accepts; \(X_{1,2}^{D}\) increases by \(1\) \\
\addlinespace
2 & 2 & \(r=2,\ \tau=2\) & balks; the day \(D+2\) capacity remains available \\
\addlinespace
3 & 1 & \(r=2,\ \tau=2\) & accepts; \(X_{1,2}^{D}\) increases by \(1\) \\
\bottomrule
\end{tabular}
\normalsize
\end{center}
...
\[
N_0^D=2,\qquad N_1^D=3,\qquad N_2^D=3,\qquad N_3^D=0.
\]
The two accepted patients are stored in the audit layer as \((1,2)\) and \((1,2)\). The balked
class-2 patient creates no appointment record.

Now the simulator resolves today's \(r=0\) appointments. Suppose one scheduled patient attends and
one no-shows. Since \(S=3\) and only two patients were scheduled for today, the daily service
outcome is:
\[
\mathrm{served}=1,\qquad
\mathrm{no\_shows}=1,\qquad
\mathrm{empty\_slots}=1.
\]
...
\midrule
cancellations & 0 & 0 & 0 & 0 & 1 & 0 & 0 & 0 & one appointment removed before arrivals \\
\addlinespace
arrivals & 3 & 0 & 0 & 0 & 1 & 0 & 0 & 0 & daily Poisson counts have been drawn \\
\addlinespace
offers and balking & 3 & 3 & 2 & 1 & 1 & 0 & 0 & 0 & all arrivals got an offer; one declined \\
\addlinespace
no-shows/service & 3 & 3 & 2 & 1 & 1 & 1 & 1 & 1 & today's scheduled patients are resolved \\
\addlinespace
metrics & 3 & 3 & 2 & 1 & 1 & 1 & 1 & 1 & daily counters are ready for summaries \\
\addlinespace
advance & 3 & 3 & 2 & 1 & 1 & 1 & 1 & 1 & old \(r=1\) becomes the new \(r=0\) \\
\bottomrule
\end{tabular}
}
\normalsize
...
\begin{itemize}[nosep]
    ...
    \item \texttt{cancel\_probability} as a scalar is the direct daily cancellation probability \(\phi_i\).
...
\begin{itemize}[nosep]
    ...
    \item Scalar \(\phi_i\) is a daily cancellation probability applied once per day.
...
larger \(\tau\) then enters \(b_i(\tau)\) and \(\xi_i(\tau)\). In the baseline, cancellation is
the scalar daily probability \(\phi_i\). In later experiments, cancellation can be replaced by the
advanced rule \(\tilde{\phi}_i(\tau,r)\).
...
congestion measure. The v2 model uses the same shape, but evaluates it on the calendar delay:
\[
\xi_i(\tau)
=
\gamma_{i,\max}
...
\begin{itemize}[nosep]
    ...
    \item their no-show mechanism maps to \(\xi_i(\tau)\);
...
\begin{itemize}[nosep]
    ...
    \item their show-up probability maps to \(1-\xi_i(\tau)\);
...
\begin{itemize}[nosep]
    ...
    \item delay-sensitive attendance maps to \(\xi_i(\tau)\);
