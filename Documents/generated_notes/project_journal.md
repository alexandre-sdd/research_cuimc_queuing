...
- Aligned the cancellation plots in [behavior_functions.ipynb](/Users/alexandresepulvedadedietrich/Documents/School/Columbia/Research/Notebooks/behavior_functions.ipynb) with the note’s convention by plotting them against normalized remaining share `r/\tau` and reversing the x-axis so the curve reads as “falling as the appointment approaches.”
- Re-executed [behavior_functions.ipynb](/Users/alexandresepulvedadedietrich/Documents/School/Columbia/Research/Notebooks/behavior_functions.ipynb) after the cancellation-plot fix to refresh its outputs and confirm the notebook now matches the note visually as well as mathematically.
- Simplified the cancellation visualization in [behavior_functions.ipynb](/Users/alexandresepulvedadedietrich/Documents/School/Columbia/Research/Notebooks/behavior_functions.ipynb) by splitting class and option into separate subplots and plotting against appointment proximity `1-r/\tau`, so the same function is easier to read and now decreases left to right as the appointment gets closer.
- Re-executed [behavior_functions.ipynb](/Users/alexandresepulvedadedietrich/Documents/School/Columbia/Research/Notebooks/behavior_functions.ipynb) after the plot redesign to refresh the outputs under the cleaner convention.
- Added an exact cumulative cancellation probability to [profiles.py](/Users/alexandresepulvedadedietrich/Documents/School/Columbia/Research/appointment_simulation/profiles.py), defined from the daily hazards through a survival product rather than a naive cumulative sum.
- Extended [behavior_functions.ipynb](/Users/alexandresepulvedadedietrich/Documents/School/Columbia/Research/Notebooks/behavior_functions.ipynb) with a dedicated cumulative-cancellation section and plot, so the notebook now shows both the daily risk `\phi_i(\tau,r)` and the probability that a patient has already canceled by a given point in the wait.
- Extended [test_simulation.py](/Users/alexandresepulvedadedietrich/Documents/School/Columbia/Research/tests/test_simulation.py) to verify the new cumulative cancellation profile and re-ran the test suite successfully.
- Extended [simulation.ipynb](/Users/alexandresepulvedadedietrich/Documents/School/Columbia/Research/Notebooks/simulation.ipynb) with a dedicated backlog-dynamics section, including day-start backlog lines, class-composition lines, heatmaps of `X_{i,r}^D`, and average backlog profiles by residual delay.
- Added a slightly heavier FCFS visualization scenario to the simulation notebook so the queue-filling mechanism is visible even though the main baseline remains lightly loaded.
- Re-executed [simulation.ipynb](/Users/alexandresepulvedadedietrich/Documents/School/Columbia/Research/Notebooks/simulation.ipynb) after the backlog-visualization update.
- Added a curated showcase section to [simulation.ipynb](/Users/alexandresepulvedadedietrich/Documents/School/Columbia/Research/Notebooks/simulation.ipynb) around a moderate-congestion FCFS case (`lambda = 1.05`) to demonstrate where the behavior functions matter.
- The showcase now compares a reference realistic case against a more delay-sensitive no-show variant and a heavier-cancellation variant while keeping FCFS, class mix, and demand fixed.
- Re-executed [simulation.ipynb](/Users/alexandresepulvedadedietrich/Documents/School/Columbia/Research/Notebooks/simulation.ipynb) after adding the showcase comparison.
